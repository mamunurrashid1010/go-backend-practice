...
// Memory is the Day 27 in-process token-bucket limiter, now returning
// a Decision so it shares an interface with the Redis-backed one.
type Memory struct {
	mu       sync.Mutex
...
func (l *Memory) Allow(_ context.Context, key string) Decision {
	...
	if !ok {
		// rate.Limiter doesn't expose a precise retry-after; approximate
		// as "1 / rps" since the bucket fills steadily.
		if l.rate > 0 {
