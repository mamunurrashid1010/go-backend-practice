...
// Repository — List now returns ListPage; everything else unchanged.
...
// List builds a dynamic query with optional search, optional cursor, and
// LIMIT N+1 — the "+1 trick" so we can tell if there's a next page without
...
func (r *PostgresRepository) List(ctx context.Context, userID int64, f ListFilter) (ListPage, error) {
	...
	if f.AfterID > 0 {
		...
		// DESC pages walk forward by id-LESS-than-cursor; ASC by GREATER-than.
		if f.SortDesc {
	...
	args = append(args, f.Limit+1) // the +1 trick
	...
	// If we got back limit+1, there's another page. Slice off the extra
	// row and emit its predecessor's id as the cursor.
	var nextID int64
