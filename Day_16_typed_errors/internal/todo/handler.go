...
import (
	...
	"day16/internal/respond"
	"day16/internal/validate"
)
...
// writeServiceErr — THE single domain-error → HTTP mapping.
//
// Uses errors.Is for sentinels (identity) and errors.As for typed errors
// (to read their fields). Both walk the wrapped chain, so a repository that
// wraps with %w is still matched here.
func writeServiceErr(w http.ResponseWriter, err error) {
	...
	default:
		// Anything unmapped: logged internally, opaque 500 to the client.
