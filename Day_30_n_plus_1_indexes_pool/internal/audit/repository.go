...
type Repository interface {
	...
	// Three implementations of the same view — see README.md §1.
...
// ListWithNotesJoin — the right answer for this shape.
// One query, LEFT JOIN so deleted notes still appear with note=null.
func (r *PostgresRepository) ListWithNotesJoin(ctx context.Context, userID int64, limit int) ([]EntryWithNote, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&ewn.ID, &ewn.UserID, &ewn.Action, &ewn.TargetType, &ewn.TargetID, &meta, &ewn.CreatedAt,
			&nID, &nTtl,
		); err != nil {
...
// ListWithNotesInBatch — two queries: list audit, then ONE SELECT
// pulling every referenced note via IN (...). Useful when the JOIN
// would be awkward (cross-database, REST microservice, codegen).
func (r *PostgresRepository) ListWithNotesInBatch(ctx context.Context, userID int64, limit int) ([]EntryWithNote, error) {
	...
	// Collect the distinct note ids we'd need to look up.
	...
	if len(idSet) > 0 {
		// Build $2, $3, ... for the IN clause. Portable across drivers.
		...
		q := fmt.Sprintf(
			"SELECT id, title FROM notes WHERE user_id = $1 AND id IN (%s)",
...
// ListWithNotesNaive — the N+1 antipattern. Kept ONLY for the
// ?strategy=naive teaching demo. One SELECT per entry — for 50 entries
// that's 51 queries. Watch the logs.
func (r *PostgresRepository) ListWithNotesNaive(ctx context.Context, userID int64, limit int) ([]EntryWithNote, error) {
	...
	for _, e := range entries {
		...
		if e.TargetType == TargetNote {
			...
			if err != nil {
				// Note may have been deleted; audit row outlives it.
				out = append(out, ewn)
				continue
			}
			ewn.Note = &n
