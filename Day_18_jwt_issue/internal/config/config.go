// Package config — Day 18 adds AuthConfig for the JWT settings.
package config
...
// AuthConfig — JWT settings. The secret MUST come from the env, never code.
type AuthConfig struct {
	JWTSecret    string        `env:"JWT_SECRET,required"`
	JWTAccessTTL time.Duration `env:"JWT_ACCESS_TTL" envDefault:"15m"`
	JWTIssuer    string        `env:"JWT_ISSUER"     envDefault:"day18-auth"`
}
...
func (c Config) validate() error {
	...
	// Defense-in-depth: refuse trivially weak secrets in any env.
