...
// PostgresRefreshTokenRepository — hand-written (not sqlc).
// The recursive CTE in RevokeFamilyDescendants is awkward to express
// in a sqlc query, and the file is small enough to leave alone.
// See Day 31 README §4.
...
func (r *PostgresRefreshTokenRepository) Revoke(ctx context.Context, id int64, replacedByID *int64) error {
	const q = `UPDATE refresh_tokens SET revoked_at = now(), replaced_by_id = $2 WHERE id = $1 AND revoked_at IS NULL`
...
func (r *PostgresRefreshTokenRepository) RevokeFamilyDescendants(ctx context.Context, startID int64) error {
	const q = `
		WITH RECURSIVE family AS (
		    ...
		    UNION ALL
		    SELECT rt.id, rt.replaced_by_id FROM refresh_tokens rt JOIN family f ON rt.id = f.replaced_by_id
		)
		UPDATE refresh_tokens SET revoked_at = now() WHERE id IN (SELECT id FROM family) AND revoked_at IS NULL
	`
