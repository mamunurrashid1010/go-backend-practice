...
// q returns a fresh *db.Queries bound to whatever runner is on ctx
// (a *sql.Tx if InTx is in flight, else the *sql.DB pool).
func (r *PostgresRepository) q(ctx context.Context) *db.Queries {
	return db.New(dbtx.RunnerFor(ctx, r.db).(db.DBTX))
}

// run is the raw dbtx runner. Used by List below, which is dynamic SQL
// and stays hand-written. See README.md §4 for why.
func (r *PostgresRepository) run(ctx context.Context) dbtx.DBTX {
	return dbtx.RunnerFor(ctx, r.db)
}

// Get / Create / Update / Patch / Delete — sqlc.
// List — hand-written (dynamic WHERE + ORDER BY + LIMIT N+1 trick).
...
func (r *PostgresRepository) Update(ctx context.Context, userID, id int64, in UpdateRequest) (Note, error) {
	row, err := r.q(ctx).UpdateNote(ctx, db.UpdateNoteParams{
		ID: id, UserID: userID, Title: in.Title, Body: in.Body,
	})
	switch {
	case errors.Is(err, sql.ErrNoRows):
		return Note{}, ErrNotFound
...
// List — dynamic WHERE based on optional search + optional cursor,
// optional sort direction, LIMIT N+1 trick. sqlc's strength is fixed
// queries; this is not one. It stays hand-written.
