...
// Service — Day 29 wires a Transactor + audit repo. Create/Patch/Delete
// now write a domain row AND an audit row in one transaction. Read
// paths (List, Get) don't need a tx; they're single statements.
type Service struct {
	repo  Repository
...
// Create — wraps INSERT note + INSERT audit in one transaction.
// If the audit insert fails the note insert is rolled back; the API
