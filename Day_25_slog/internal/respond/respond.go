// Package respond — Day 25 change: Internal takes a context so it can pull
// the request-scoped logger off it instead of using package-level log.
package respond
...
func JSON(w http.ResponseWriter, status int, v any) {
	...
	if err := json.NewEncoder(w).Encode(v); err != nil {
		// We're past WriteHeader; just log to the default logger.
		slog.Default().Error("respond.JSON encode error", slog.Any("err", err))
	}
}
...
// Internal logs the real error against the request-scoped logger and
// returns an opaque body to the client. Never leak err.Error() to the wire.
