...
type RefreshTokenRepository interface {
	...
	// RevokeFamilyDescendants walks replaced_by_id forward from startID and
	// revokes every still-active descendant. Used on reuse detection — if a
	// REVOKED token is presented again, the whole chain that descends from
	// it gets killed.
...
// RevokeFamilyDescendants uses a recursive CTE to walk the replaced_by_id
// chain forward and revoke every still-active descendant in one shot.
//
// Pictorially, if the chain is:
//     startID -> A -> B -> C
// then this revokes A, B, C (whichever are still active). startID itself
// is already revoked when this is called.
