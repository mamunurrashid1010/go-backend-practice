...
type AuthConfig struct {
	...
	JWTIssuer    string        `env:"JWT_ISSUER" envDefault:"day27-auth"`
	RefreshTTL   time.Duration `env:"REFRESH_TTL" envDefault:"720h"`
}

// RateLimitConfig — separate global vs auth-route limits.
// Global is loose (typical API traffic). Auth is tight (brute-force).
...
// CORSConfig — list-driven so it's wired through .env.
...
func (c Config) validate() error {
	...
	// CORS sanity: wildcard + credentials is illegal per the spec.
